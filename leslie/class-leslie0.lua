--[[
  ...
  This is a modified version of class library for lua.

  o class environment - default is global

  class "Foo"

  ex. class is added to module environment
...
local
function object2class(o,name,env)
  assert(isobject(o), fwrongarg(1,'object',o))
  assert(isname(name), fwrongarg(2,'name',name))
  givename(o,name,env)
...
function Class:initialize(name,superclass,env)
  assert(isname(name), fwrongarg(1,'name',name))
  object2class(self,name,env)
...
function class(name, env)
  assert(isname(name), fwrongarg(1,'name',name))
  local _class = Class(name, nil, env)
