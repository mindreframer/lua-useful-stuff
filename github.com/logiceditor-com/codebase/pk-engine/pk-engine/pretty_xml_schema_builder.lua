--------------------------------------------------------------------------------
-- pretty_xml_schema_builder.lua - build xml schema for pretty-printing
...
do
  local walk_schema = function(schema, indent, cat, data)
    for i = 1, #schema do
      local res, err = (schema[i])(indent + 1, cat, data)
      if res == nil then
  ...
  local attribute_renderer = function(
    ...
    return function(indent, cat, t)
      ...
      if v ~= nil then
        cat (INDENT[indent]) (node_name) '="' (filter(v)) '"\n'
      elseif is_required then
  ...
  local node_open_renderer = function(node_name)
    ...
    return function(indent, cat, t)
      cat (INDENT[indent - 1]) '<' (node_name) -- Hack
      return true
    end
  end

  -- Note this is non-dynamic function
  local node_open_close_renderer = function(indent, cat, t)
    cat '>\n'
    return true
  end

  -- Note this is non-dynamic function
  local node_close_renderer_singleton = function(indent, cat, t)
    cat '/>\n'
    return true
  end
  ...
  local node_renderer = function(is_required, key, node_name, schema)
    ...
    return function(indent, cat, t)
      local v = getter(t, key)
      if v ~= nil then
        local res, err = walk_schema(schema, indent, cat, v)
        if res == nil then
          return nil, "node: " .. err
        end
      elseif is_required then
  ...
  local list_renderer = function(
    ...
    return function(indent, cat, t)
      local v = getter(t, key)
      if v ~= nil then
        cat (INDENT[indent]) '<' (node_name)
        if list_attribute_renderer then
          cat '\n'
          local res, err = list_attribute_renderer(indent, cat, t)
          if res == nil then
        ...
        for k, v in iterator(v) do
          local res, err = renderer(indent + 1, cat, v)
          if res == nil then
  ...
  local const_attribute = function(self, node_name, value)
    ...
    local fn = invariant_renderer(
        node_name..'="'..htmlspecialchars(tostring(value))..'"\n'
      )
    self.attributes_[fn] = true
    return fn
  end
  ...
  local postindent_renderer = function(before, indent_more, after)
    return function(indent, cat, t)
      cat (before) (INDENT[indent + indent_more]) (after)
      return true
    end
  end
      ...
      return function(schema)
        ...
        for _, data in ipairs(schema) do
          if self.attributes_[data] then
            if #attributes == 1 then -- Hack
              attributes[#attributes + 1] = invariant_renderer('\n')
            end
        ...
        if #subnodes > 0 then
          if #attributes > 1 then
            attributes[#attributes + 1] = postindent_renderer('', -1, '  ')
          end
  ...
  make_pretty_xml_schema_builder = function()
    return
    {
      raw = raw;
...
return
{
  make_pretty_xml_schema_builder = make_pretty_xml_schema_builder;
}
