--------------------------------------------------------------------------------
-- pretty_json_schema_builder.lua - build xml schema
...
local log, dbg, spam, log_error = import 'pk-core/log.lua' { 'make_loggers' }(
    "pretty_json_schema_builder.lua", "PJB"
  )
...
-- NOTE: Based on pretty_xml_schema_builder.
...
do
  ...
  local attribute_renderer = function(
    ...
    return function(indent, cat, t)
      ...
      if v ~= nil then
        cat.add '\n' (INDENT[indent]) '"' (node_name) '": ' (filter(v))
        was_output = true
      elseif is_required then
  ...
  local node_renderer = function(
    ...
    local getter = getters[key ~= nil]
    return function(indent, cat, t, caption_renderer)
      local v = getter(t, key)
      local was_output = false
      if v ~= nil then
    ...
    return function(indent, cat, t)
      ...
      if v ~= nil then
        ...
        if list_attribute_renderer then
          local res, err = list_attribute_renderer(indent, cat, t, false)
          if res == nil then
        ...
        for k, v in iterator(v) do
          ...
          local res, err = renderer(indent + 2, cat, v, caption_renderer)
          cat.add "\n" (INDENT[indent + 2]) "}"
          if res == nil then
  ...
  make_pretty_json_schema_builder = function()
    return
    {
      raw = raw;
...
return
{
  make_pretty_json_schema_builder = make_pretty_json_schema_builder;
}
