...
local log, dbg, spam, log_error = make_loggers("pk-core/dsl_loader", "DLD")
...
do
  ...
  local tertiary_mt =
  {
    ...
    __index = function(t, v)
      -- spam("TERTIARY", v, debug.traceback("here"))
    ...
    __call = function(t, data, ...)
      ...
      --assert_is_table(data, "data should be a table", 2)
  ...
  local secondary_mt =
  {
    ...
    __index = function(t, v)
      -- spam("SECONDARY", v, debug.traceback("here"))
      error("you should call this object, not index it", 2)
    end;
  ...
  make_dsl_loader = function(name_filter, data_filter)
    data_filter = data_filter or tappend_many
...
-- TODO: Data-only calls should be made by filter means only
